...
func loginHandler() iris.Handler {
	return func(ctx *context.Context) {
		...
		u, err := userService.GetByUserName(loginData.Username, common.DBOptions{})
		errk := pwderrKey + loginData.Username
		...
		if !utils.ComparePwd(loginData.Password, u.Password) {
			utils.ErrorStr(ctx, consts.Pwderrstr)
			utils.Set(errk, errcount+1, lockTime)
			return
		}
		...
		userinfo := &model.Userinfo{
			...
			Email:     u.Email,
			Phone:     u.Phone,
			LastLogin: u.LastLogin.Format(consts.Custom),
		}
		...
		go func() {
			// 更新最后登录日志
			_ = userService.Update(u, common.DBOptions{})
