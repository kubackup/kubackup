		...
		if len(snapshotIDs) != 0 {
			...
			for _, s := range snapshotIDs {
				if s == "latest" {
					...
					if err != nil {
						server.Logger().Warnf("Ignoring %q, no snapshot matched given filter (Paths:%v Tags:%v Hosts:%v)\n", s, paths, tags, hosts)
						continue
					}
				} else {
			...
			if !usedFilter && (len(hosts) != 0 || len(tags) != 0 || len(paths) != 0) {
				server.Logger().Warnf("Ignoring filters as there are explicit snapshot ids given\n")
			}

			for _, id := range ids.Uniq() {
				sn, err := restic.LoadSnapshot(ctx, repo, id)
				if err != nil {
					server.Logger().Warnf("Ignoring %q, could not load snapshot: %v\n", id, err)
					continue
				}
				select {
				case <-ctx.Done():
